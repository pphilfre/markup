...
function parseAdmonition(children: React.ReactNode): { type: string; content: React.ReactNode } | null {
  // Look for the pattern [!TYPE] in the first child paragraph
  ...
  // If remaining text is empty and no other parts, use rest of children
  const newFirst = newFirstParts.length > 0
    ? { ...first, props: { ...first.props, children: newFirstParts.length === 1 ? newFirstParts[0] : newFirstParts } }
    : null;
