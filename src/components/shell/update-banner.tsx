...
import {
  ...
  checkForDockerUpdate,
  getDockerUpdateDocsUrl,
  getDockerUpdateState,
  ignoreDockerUpdate,
  subscribeToDockerUpdateState,
} from "@/lib/docker-updater";
import { isDocker, isTauri, openExternal } from "@/lib/tauri";

export function UpdateBanner() {
  const [state, setState] = useState(getUpdateState);
  const [dockerState, setDockerState] = useState(getDockerUpdateState);
  ...
  useEffect(() => {
    if (!isDocker()) return;
    const unsub = subscribeToDockerUpdateState(() => setDockerState(getDockerUpdateState()));
    return unsub;
  }, []);
  ...
  if (!isTauri() && !isDocker()) return null;
  if (dismissed) return null;

  const currentVersion = packageJson.version;

  if (isDocker()) {
    const { status, info, error } = dockerState;
        ...
        <div className="fixed bottom-4 right-4 z-50 flex flex-col gap-2 rounded-lg border border-border bg-popover px-3 py-2 text-xs shadow-lg backdrop-blur-sm max-w-xs">
          <div className="flex items-center gap-2">
            ...
            <div className="flex-1 min-w-0">
              <p className="font-medium">Docker update available: v{info?.version}</p>
            ...
            <Button
              ...
              variant="ghost"
              className="text-xs h-6 px-2 shrink-0"
              onClick={() => ignoreDockerUpdate(info?.version ?? null)}
            >
              Ignore
            </Button>
